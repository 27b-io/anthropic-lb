...
| `readers` | `Vec<String>` | `[]` | Client IDs granted `/_stats` + `/metrics` and nothing else — every `/v1` surface answers 403. The role for a scrape or a dashboard. Requires `[[clients]]`; a name in both `readers` and `operators` is a startup error |
...
| **Read-only principal** | `readers = ["vmagent"]` | Same two surfaces, but `/v1` answers 403 — a scrape key that cannot spend the pool | scrapes need an operator key, which bypasses every limit |
