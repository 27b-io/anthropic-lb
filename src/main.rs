...
async fn classify_retry_status(
    ...
    if status == StatusCode::TOO_MANY_REQUESTS {
        ...
        // would be worse: although `x-should-retry` is reflected to callers
        // as the transient hint, skipping rotation would leave the account
        // pinned — and standard traffic routed to that same account
