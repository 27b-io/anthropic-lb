...
| `clients[].preferred_endpoints` | string[]? | [] (no pin) | Endpoint names this client is pinned to (LAB-2636/#151). While ≥1 is healthy (serves the model, not hard-limited/transport-unhealthy, `gate < soft_limit`), routing restricts to this set; otherwise the request spills to the full pool. Entries must name configured endpoints (startup-validated). `proxied` logs carry `pin=pinned\|spilled\|-` |
