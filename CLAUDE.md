...
| `auth_failure_limit` | u32? | 10 | Failed-auth attempts per client IP in the window before further invalid credentials get 429 + `retry-after`; valid credentials always pass. 0 disables. Counted in `anthropic_auth_failures_total{route,cred,ua}` and logged with `src_port`, `cred` (presented-credential header shape), a one-way `key_fp` and a bounded `ua` (LAB-4720); state bounded at 4096 IPs; eviction purges expired windows first, then the least-established live entry (lowest count, oldest window as tie-breaker) so fresh-failure floods can't flush an active lockout |
